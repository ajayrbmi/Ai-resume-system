    ...
    <nav className={`fixed w-full top-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white/90 backdrop-blur-md shadow-sm border-b border-gray-100 py-3' : 'bg-transparent py-5'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center">
          <Link to="/" className="text-2xl font-extrabold tracking-tight text-gray-900 flex items-center gap-2">
          ...
          <div className="hidden md:flex items-center gap-8">
            <nav className="flex gap-6 text-sm font-medium text-gray-600">
              <Link to="/" className="hover:text-primary-600 transition-colors">Resume Templates</Link>
      ...
      {mobileMenu && (
        <div className="md:hidden absolute top-full left-0 w-full bg-white border-b shadow-xl px-4 py-6 flex flex-col gap-4">
          <Link to="/" className="font-medium text-lg text-gray-800" onClick={() => setMobileMenu(false)}>Resume Templates</Link>
